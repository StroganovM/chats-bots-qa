...
class StoragePostgres(Storage):
    ...
    async def _get_pool(self) -> asyncpg.Pool:
        if self._pool is None:
            ...
            if port is None:
                raise ValueError("POSTGRES_HOST_PORT environment variable is not set")
            if user is None:
    ...
    async def persist_update(self, update: dict) -> None:
        ...
        try:
            ...
            duration_ms = (time.time() - start_time) * 1000
            logger.error(f"[DB] X {method_name} - {duration_ms:.2f}ms - Error: {e}")
            raise

    async def update_user_order_json(self, telegram_id: int, order: dict) -> None:
    ...
    async def clear_user_order_and_state(self, telegram_id: int) -> None:
